...
float initialBrushRadius = 4800.0;
...
float probabilityBlendAdd = 0.94;
float probabilityBlendIncrement = 0.02;

float idealIntensity = 220.0;//dark: 60 mid: 120 lighter:160 high:220

float[] tintMultiplier = {2.0, 2.0, 2.0};
float tintMultiplierIncrement = 0.4;
...
void draw() {
  ...
  //}else{
  drawer.ApplyForce(
    (noise(perlinOffsetX, 0)-0.5)*35.0, 
    (noise(0, perlinOffsetY)-0.5)*35.0
  ...
  if (currentStep>steps) {
    ...
    if (currentStep<steps*0.1) {
      ...
      brushRadius = initialBrushRadius/2;
      opacity =  (noise(perlinOffsetX+0.5)+0.05)*3.0*((1-(currentStep/steps))*0.05 + 0.01)*initialOpacity;
    }
  }
  imageMode(CORNER);
  image(pg, 0, 0, width, height);
...
void drawMemesAtPosition(int x, int y) {
  ...
  for (int i = 0; i < numberImages; i++) {
    ...
    pg.tint(
      noise(perlinOffsetX+17.1)*255*tintMultiplier[0], 
      noise(perlinOffsetX/2.0 + perlinOffsetY/2.0 + 19.2)*255*tintMultiplier[1], 
      noise(perlinOffsetY+21.3)*255*tintMultiplier[2], 
      opacity
      );
